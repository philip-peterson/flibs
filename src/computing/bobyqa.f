C     bobyqa.f --
C
C     Powell's BOBYQA algorithm, turned into a module, interface changes
C
      MODULE BOBYQA_MINIMISATION
      ...
      SUBROUTINE BOBYQA (NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,
     1  MAXFUN,CALFUN)
      DIMENSION X(:),XL(:),XU(:)
...
C
      N=SIZE(X)
      IF (SIZE(XL) .NE. SIZE(XU) .OR. SIZE(XL) .NE. SIZE(X)) THEN
          PRINT 5
    5     FORMAT (/4X,'Return from BOBYQA because X, XL, XU are',
     1      ' not all the same length')
          GO TO 40
      ENDIF
...
C
   ...
   40 RETURN
     ...
     2  SL,SU,XNEW,XALT,D,VLAG,W,CALFUN)
      DIMENSION X(N),XL(*),XU(*),XBASE(*),XPT(NPT,*),FVAL(*),
...
C
      ...
      IF (MAX(DELTA,DNORM) .GT. RHO) GOTO 60
C
...
C
  ...
  200 RETURN
     ...
     1  XPT,FVAL,GOPT,HQ,PQ,BMAT,ZMAT,NDIM,SL,SU,NF,KOPT,CALFUN)
      DIMENSION X(N),XL(*),XU(*),XBASE(*),XPT(NPT,*),FVAL(*),GOPT(*),
...
C
      ...
      IF (PTSAUX(1,J)+PTSAUX(2,J) .LT. ZERO) THEN
          ...
          PTSAUX(2,J)=TEMP
      END IF
      IF (ABS(PTSAUX(2,J)) .LT. HALF*ABS(PTSAUX(1,J))) THEN
...
C
      JL=1
      DO 30 J=2,NPTM
      IF (ABS(ZMAT(KNEW,J)) .GT. ZTEST) THEN
...
C
   ...
   60 CONTINUE
      ...
      END MODULE BOBYQA_MINIMISATION
