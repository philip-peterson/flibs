...
& scan_date         ! Scan a string for a date/time
...
! scan_date --
!     Scan a string for date/time information
!
! Arguments:
!     pattern          String that serves as the pattern
!     string           Resulting string
!     date             Date/time to be formatted
!     error            An error occurred during parsing or it was successful
!
! Note:
!     The pattern can contain any of the following substrings
!     that will be used to interpret the string
!
!     dd        Day of month ("01" for instance)
!     ds        Day of month ("1" for instance, s for space)
!     DDD       Day of the year
...
!     MS        Minutes within the hour (0-59)
!     YY        Year without the century
!     yyyy      Year with the century (1 to 4 digits expected,
!               even though the Gregorian calendar has some quirks!)
!     ?         Skip one character
!     (space)   Skip one or more spaces
...
! (Added by Arjen Markus)
   ...
   SUBROUTINE scan_date( pattern, string, date, error )

   character(len=*), INTENT(IN)  :: pattern
   character(len=*), INTENT(IN)  :: string
   TYPE(DateType), INTENT(INOUT) :: date
   logical                       :: error

   character(len=len(pattern))   :: pattern_scan
   character(len=len(string))    :: string_scan
   integer                       :: ip, lenp
   integer                       :: is, lens
   logical                       :: found

   error        = .false.
   string_scan  = adjustl(string)
   pattern_scan = adjustl(pattern)

   lenp         = len_trim(pattern_scan)
   lens         = len_trim(string_scan)

   ip           = 1
   is           = 1

scan_loop: &
   do while ( ip <= lenp .and. .not. error )
       select case ( pattern_scan(ip:ip) )
           case ( ' ' )
               skip_spaces: &
                   do is = is+1,lenp
                       if ( string_scan(is:is) /= ' ' ) then
                           ip = ip + 1
                           cycle scan_loop
                       endif
                   enddo &
           ...
           case ( 'd' )
               ip = ip + 1
               if ( pattern_scan(ip:ip) == 'd' ) then
                   call get_number( string_scan, is, 2, 2, date%day, error )
               elseif ( pattern_scan(ip:ip) == 's' ) then
                   call get_number( string_scan, is, 1, 2, date%day, error )
               else
                   error = .true.
               endif
               ip = ip + 1
           ...
           case ( 'H' )
               ip = ip + 1
               if ( pattern_scan(ip:ip) == 'H' ) then
                   call get_number( string_scan, is, 2, 2, date%hour, error )
               elseif ( pattern_scan(ip:ip) == 'S' ) then
                   call get_number( string_scan, is, 1, 2, date%hour, error )
               else
                   error = .true.
               endif
               ip = ip + 1
           ...
           case ( 'M' )
               ip = ip + 1
               if ( pattern_scan(ip:ip) == 'M' ) then
                   call get_number( string_scan, is, 2, 2, date%minute, error )
               elseif ( pattern_scan(ip:ip) == 'S' ) then
                   call get_number( string_scan, is, 1, 2, date%minute, error )
               else
                   error = .true.
               endif
               ip = ip + 1
           ...
           case ( 'Y' )
               ip = ip + 1
               if ( pattern_scan(ip:ip) == 'Y' ) then
                   call get_number( string_scan, is, 2, 2, date%year, error )
           ...
           case ( 'y' )
               if ( pattern_scan(ip:ip+3) == 'yyyy' ) then
                   call get_number( string_scan, is, 1, 4, date%year, error )
                   ip = ip + 4
               else
                   error = .true.
               endif
           ...
           case ( 'm' )
               ip = ip + 1
               if ( pattern_scan(ip:ip) == 'm' ) then
                   call get_number( string_scan, is, 2, 2, date%month, error )
               elseif ( pattern_scan(ip:ip) == 's' ) then
                   call get_number( string_scan, is, 1, 2, date%month, error )
               else
                   error = .true.
               endif
               ip = ip + 1
           ...
           case default
               if ( string_scan(is:is) /= pattern_scan(ip:ip) ) then
                   error = .true.
               endif
               ip = ip + 1
               is = is + 1
...
scan_loop
   ...
   END SUBROUTINE scan_date

!
! Private routine to find a number in a date/time string
!
! Arguments:
!     string              String to be scanned
!     pointer             At entrance first character to be examined, on exit:
!                         last character that was examined (therefore the
!                         string pointer is positioned correctly for the next scan)
!     minlength           Minimum length of the substring containing the number
!     maxlength           Maximum length
!     number              Numerical value returned (note: set to zero on error)
!     error               Whether there was an error or not
!
   SUBROUTINE get_number( string, pointer, minlength, maxlength, number, error )
   character(len=*), intent(in)   :: string
   ...
   do p = pointer, pointer+maxlength-1
       if ( index( '0123456789', string(p:p) ) <= 0 ) then
           exit
       endif
   enddo

   if ( p < pointer+minlength ) then
       error = .true.
       number = 0
   else
       read( string(pointer:p-1), *, iostat = ierr ) number
       if ( ierr /= 0 ) then
           error = .true.
           number = 0
       endif
