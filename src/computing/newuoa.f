C     newuoa.f --
C
C     Powell's NEWUOA algorithm, turned into a module, interface changes
C
      MODULE NEWUOA_MINIMISATION
      ...
      SUBROUTINE NEWUOA (NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,CALFUN)
...
C
   ...
   20 RETURN
     ...
     1  XOPT,XNEW,XPT,FVAL,GQ,HQ,PQ,BMAT,ZMAT,NDIM,D,VLAG,W, CALFUN)
      DIMENSION X(N),XBASE(*),XOPT(*),XNEW(*),XPT(NPT,*),FVAL(*),
...
C
  ...
  150 S(I)=GC(I)+GD(I)
      IF (ABS(TAU) .LE. 1.1D0*ABS(TAUBEG)) GOTO 160
...
C
   ...
   70 CONTINUE
      ...
      END MODULE NEWUOA_MINIMISATION
